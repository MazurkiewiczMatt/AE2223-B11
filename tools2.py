...
def range_calc2(PSD, L, freq, chirp_time):
    PSD2 = PSD[L[0]:L[-1]]
    freq2 = freq[L[0]:L[-1]]
    ...
    for k in range(len(freq2)):
        if PSD2[k] > 0:
            range_lst = np.append(range_lst, c * T * freq2[k] / (2 * B))
    return range_lst
    
def angle_calc(PSD, L, freq, phi_1, phi_2):
    PSD2 = PSD[L[0]:L[-1]]
    freq2 = freq[L[0]:L[-1]]
...
def range_calc(PSD, L, freq, chirp_time, phi_1, phi_2):
    ...
    return range_lst, geo_angle_lst


def fourier(chirps, t, realim, duration, threshold):  # from the internet
